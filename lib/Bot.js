  ...
  async updateBsp(before) {
    try {
      ...
      const [url, messageId] = await new Promise(async (resolve, reject) => {
        try {
          const find7z = async (resolve, before) => {
            const messages = (await this.channel.filter(before ? { before } : 523676479389696000)).array();
            ...
            for (let i in messages) {
              ...
              for (let j in attachments) {
                ...
                if (!/^.*\.7z/.test(attachment.filename)) continue;
                resolve([attachment.url, message.id]);
                return;
              }
            }
            await find7z(resolve, messages[messages.length - 1].id);
          };
          await find7z(resolve, before);
      ...
      try {
        ...
        const rar = _path2.default.resolve(tmpDir, '7z');
